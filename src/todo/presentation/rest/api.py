...
def get_todo_handler(request, todo_id: int):
    ...
    return 200, {"todo": ToDoResponse.build_response(todo=todo)}
...
def get_todo_list_handler(request):
    ...
    return 200, {"todos": [ToDoResponse.build_response(todo=todo) for todo in todos]}
...
def post_todos_handler(request, body: PostToDoRequestBody):
    ...
    return 201, {"todo": ToDoResponse.build_response(todo=todo)}
